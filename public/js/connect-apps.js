...
$(document.body).on('click', '.connect-app' ,function() {
    ...
    else
    {
        ...
        if (actionType === 'google places') {
        html += '<input id="website" name="website" type="text" class="form-control input-lg google_suggestion" data-required="true" data-message="Invalid URL">';
    ...
    if (actionType === 'google places') {
        var x = document.getElementsByClassName('google_suggestion')[0];
        console.log(x);
        var places = new google.maps.places.Autocomplete(x);
        google.maps.event.addListener(places, 'place_changed', function () {
            var selectedPlace = places.getPlace();
            x.value = selectedPlace.url;
        });
        x.addEventListener("blur", function (param) { 
            x.focus();
            x.setAttribute("data-message", "");
        }, true); 
    }
